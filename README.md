...
Contributing to [Kubernetes](https://github.com/kubernetes/kubernetes) — patches around kubelet node lifecycle and pod eviction, contextual logging, and validation/codegen internals, currently in review.

Also contributing to [Nacos](https://github.com/nacos-group), focusing on keeping the Node.js and Go SDKs consistent with the Java one: a patch adding `selectOneHealthyInstance` to the Node.js SDK has been merged, with more on local caching, subscription and publish semantics in review.

Enjoying the community side of open source and interested in taking on more wherever I can be useful.
